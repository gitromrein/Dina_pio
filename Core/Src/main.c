...
int main(void)
{
  ...
  /* add semaphores, ... */
//  netSemaphoreHandle = osSemaphoreNew(1, 1, &netSemaphore_attributes);
  ...
  /* add queues, ... */
  canUdpQueueHandle = osMessageQueueNew (10, sizeof(ZMV_CAN_FRAME_Rx_U), &canUdpQueue_attributes);
  ...
  /* creation of websTask */
//  osThreadDef(webs, websTask, osPriorityNormal, 0, 512);
//  websTaskHandle = osThreadCreate(osThread(webs), NULL);
  websTaskHandle = osThreadNew(websTask, NULL, &websTask_attributes);

  /* creation of udpTask */
//  osThreadDef(udps, udpsTask, osPriorityNormal, 0, 512);
//  udpTaskHandle = osThreadCreate(osThread(udps), NULL);
  udpsTaskHandle = osThreadNew(udpsTask, NULL, &udpsTask_attributes);

  /* creation of can1Task */
//  osThreadDef(can1, can1Task, osPriorityNormal, 0, 512);
//  can1TaskHandle = osThreadCreate(osThread(can1), NULL);
  can1TaskHandle = osThreadNew(can1Task, NULL, &can1Task_attributes);

  /* creation of dinaCxTask */
//  osThreadDef(can2, can2Task, osPriorityNormal, 0, 512);
//  can2TaskHandle = osThreadCreate(osThread(can2), NULL);
  can2TaskHandle = osThreadNew(can2Task, NULL, &can2Task_attributes);

  /* creation of dinaCxTask */
//  osThreadDef(usart2, usart2Task, osPriorityNormal, 0, 512);
//  can2TaskHandle = osThreadCreate(osThread(usart2), NULL);
...
static void MX_FDCAN1_Init(void)
{
  ...
  hfdcan1.Init.RxBuffersNbr = 6;
  hfdcan1.Init.RxBufferSize = FDCAN_DATA_BYTES_8;
  hfdcan1.Init.TxEventsNbr = 0;
  hfdcan1.Init.TxBuffersNbr = 6;
